...
es_create_index_endpoint = "http://127.0.0.1:9222/{}/_doc/_bulk"
...
def send_2_es(data, index_name):
	return requests.post(es_create_index_endpoint.format(index_name), headers={'Content-Type': 'application/json'}, params=(('pretty', ''),), data=data)
...
def hello(filename, dtime_formatted_win_OS):
  ...
  data = []
  data.append(json.loads(rec.FinalResult())['text'])
  with open('ts{}.csv'.format(dtime_formatted_win_OS), 'a') as of:
	  writer = csv.writer(of)
	  writer.writerow(data)
